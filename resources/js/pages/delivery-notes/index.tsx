...
export default function DeliveryNotesIndex({ deliveryNotes, filters }: Props) {
    ...
    const submitSearch = (event: FormEvent) => {
        ...
        router.visit(
            deliveryNotesRoutes.index.url({ query: { search: search.trim() } }),
    ...
    const confirmDelete = () => {
        ...
        deleteForm.delete(deliveryNotesRoutes.destroy.url(deleteId), {
            onSuccess: () => {
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Kelola Delivery Note sebelum dipakai Invoice.
                        </p>
                    </div>
                    <Link href={deliveryNotesRoutes.create.url()}>
                        <Button>
                    ...
                    <CardContent>
                        ...
                        >
                            <div className="relative flex-1">
                                ...
                                <Input
                                    className="pl-10"
                                    placeholder="Cari nomor DN, PO, perusahaan, customer..."
                                    value={search}
                                    onChange={(event) =>
                                        ...
                                        deliveryNotes.data.map(
                                            (note, index) => (
                                                <TableRow key={note.id}>
                                                    ...
                                                    <TableCell>
                                                        <span
                                                            className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${note.status === 'available' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300'}`}
                                                        >
                                                            {note.status ===
                                                            'available'
                                                                ? 'AVAILABLE'
                                                    ...
                                                    <TableCell className="text-center">
                                                        <div className="flex justify-center gap-2">
                                                            <a
                                                                href={deliveryNotesRoutes.print.url(
                                                                    note.id,
                                                                )}
                                                                target="_blank"
                                                                rel="noreferrer"
                                                            ...
                                                            'available' ? (
                                                                <>
                                                                    <Link
                                                                        href={deliveryNotesRoutes.edit.url(
                                                                            note.id,
                                                                        )}
                                                                    >
                                                                        <Button
                        ...
                        {deliveryNotes.last_page > 1 && (
                            <Pagination
                                ...
                                baseUrl={deliveryNotesRoutes.index.url()}
                                searchParams={{ search }}
                            />
                        )}
                    </CardContent>
...
DeliveryNotesIndex.layout = {
    breadcrumbs: [
        { title: 'Transaksi', href: '#' },
        { title: 'Delivery Note', href: deliveryNotesRoutes.index.url() },
    ],
};
