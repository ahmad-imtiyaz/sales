...
export default function DeliveryNoteForm({ companies, customers, products, deliveryNote, mode }: Props) {
    ...
    return <form onSubmit={submit} className="space-y-6">
        <Card><CardHeader><CardTitle>Informasi Delivery Note</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2"><Label>Perusahaan *</Label><Select name="company_id" value={String(form.data.company_id || '')} onValueChange={(value) => form.setData('company_id', Number(value))} required><SelectTrigger className="w-full"><SelectValue placeholder="Pilih perusahaan" /></SelectTrigger><SelectContent>{companies.map((company) => <SelectItem key={company.id} value={String(company.id)}>{company.nama}</SelectItem>)}</SelectContent></Select>{form.errors.company_id && <p className="text-sm text-destructive">{form.errors.company_id}</p>}</div>
            <div className="space-y-2"><Label>Customer *</Label><Select name="customer_id" value={String(form.data.customer_id || '')} onValueChange={(value) => form.setData('customer_id', Number(value))} required><SelectTrigger className="w-full"><SelectValue placeholder="Pilih customer" /></SelectTrigger><SelectContent>{customers.map((customer) => <SelectItem key={customer.id} value={String(customer.id)}>{customer.nama}</SelectItem>)}</SelectContent></Select>{form.errors.customer_id && <p className="text-sm text-destructive">{form.errors.customer_id}</p>}</div>
